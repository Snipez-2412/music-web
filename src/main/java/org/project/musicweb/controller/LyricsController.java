...
public class LyricsController {
    ...
    @GetMapping("/{songId}")
    public ResponseEntity<LyricsEntity> getLyrics(@PathVariable Long songId) {
        Optional<LyricsEntity> lyrics = lyricsService.getLyricsBySongId(songId);
        return lyrics.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping("/add/{songId}")
    public ResponseEntity<LyricsEntity> addLyrics(@PathVariable Long songId, @RequestBody String content) {
        LyricsEntity lyrics = lyricsService.addLyrics(songId, content);
        return ResponseEntity.ok(lyrics);
    }

    @PutMapping("/update/{songId}")
    public ResponseEntity<LyricsEntity> updateLyrics(@PathVariable Long songId, @RequestBody String newContent) {
        LyricsEntity updatedLyrics = lyricsService.updateLyrics(songId, newContent);
